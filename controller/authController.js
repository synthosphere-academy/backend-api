...
const get_offline = async (req, res) => {
    try {
...
const get_onlinepayment = async (req, res) => {
    try {
        // res.status(200).json({ message: "This api working fine" });
...
const register = async (req, res) => {
    try {
        ...
        if (userEmailExist) { return res.status(400).json({ message: "This email is already Registered." }); }

        const userPhoneExist = await User.findOne({ phoneno });
        if (userPhoneExist) { return res.status(400).json({ message: "This phone number is already Registered." }); }
